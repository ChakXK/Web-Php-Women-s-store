	...
	public function index() {
		if (isset($this->request->get['token']) && isset($this->session->data['token']) && ($this->request->get['token'] == $this->session->data['token'])) {
			...
			*/	
			...
			//Header
				if ($this->user->hasPermission('access', 'catalog/headerlist')) {
				$design[] = array(
					'name'	   => $this->language->get('text_header_list'),
					'href'     => $this->url->link('catalog/headerlist', 'token=' . $this->session->data['token'], true),
					'children' => array()		
				);
			}
